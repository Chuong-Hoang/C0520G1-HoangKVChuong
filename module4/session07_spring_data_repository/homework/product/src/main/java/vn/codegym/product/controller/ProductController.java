...
public class ProductController {
    ...
    @GetMapping
    public ModelAndView getListPage(@PageableDefault(value = 8) Pageable pageable){
    ...
    @PostMapping("/create")
    public ModelAndView getCreated(@ModelAttribute Product el, @RequestParam int cate, RedirectAttributes redirect){
        ModelAndView modelAndView = new ModelAndView(REDIRECT_TO_LIST);
        el.setCategory(categoryService.findById(cate));
    ...
    @PostMapping("/edit")
    public ModelAndView getEdited(@ModelAttribute Product el, @RequestParam int cate, RedirectAttributes redirect){
        ModelAndView modelAndView = new ModelAndView(REDIRECT_TO_LIST);
        el.setCategory(categoryService.findById(cate));
        productService.save(el);
    ...
    // Search
    @PostMapping("/search")
    public ModelAndView getSearched(@RequestParam String inputSearch, @PageableDefault(value = 5) Pageable pageable){
        ModelAndView modelAndView = new ModelAndView(LIST_PAGE);
        Page<Product> elFound = productService.findByName(inputSearch, pageable);
    ...
    public ModelAndView getCreateOnListDone(@ModelAttribute Product elNew, @RequestParam int cateCreate, RedirectAttributes redirect){
        ModelAndView modelAndView = new ModelAndView(REDIRECT_TO_LIST);
        elNew.setCategory(categoryService.findById(cateCreate));
    ...
    public ModelAndView getEditOnListDone(@ModelAttribute Product elEdit, @RequestParam int cateEdit, RedirectAttributes redirect){
        ModelAndView modelAndView = new ModelAndView(REDIRECT_TO_LIST);
        elEdit.setCategory(categoryService.findById(cateEdit));
